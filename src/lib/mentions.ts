// Freeform context is stored as a portable plaintext string with inline mention
// tokens: `@[Label](projectId)`. The label is a snapshot (so a renamed/deleted
// project still reads sensibly) and the id is the durable link the lineage graph
// is derived from. Greppable and readable outside Atlas.
...
/** All ids referenced by a body — the edges the lineage graph is derived from. */
